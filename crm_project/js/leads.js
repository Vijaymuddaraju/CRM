...
function renderLeadsPage() {
  ...
  ["searchInput", "statusFilter", "sourceFilter", "assignedFilter", "priorityFilter", "dateFromFilter", "dateToFilter"].forEach(id => {
...
function populateFilterOptions() {
  ...
  document.getElementById("sourceFilter").innerHTML = `<option value="">All Sources</option>` + DB.settings.leadSources.map(s => `<option value="${s}">${s}</option>`).join("");
...
function renderLeadRow(l) {
  ...
  return `
    <tr>
      ...
      <td class="cell-muted">${formatDate(l.lastContact)}</td>
