  ...
  document.getElementById("leadInfoCard").innerHTML = `
    ...
    <div class="panel-body">
      <div class="form-grid" style="row-gap:14px;">
        ...
        ${infoField("Lead Type", lead.leadType)}
...
const MEETING_TYPE_ICON = { "Virtual Meeting": "meeting", "Client Site Visit": "building", "Dealer Visit": "briefcase" };
function renderMeetingTimeline(lead) {
  const items = DB.meetings.filter(m => m.leadId === lead.id).sort((a, b) => new Date(b.date + " " + b.time) - new Date(a.date + " " + a.time));
  document.getElementById("meetingTimeline").innerHTML = items.length ? `<div class="timeline">${items.map(m => `
    <div class="timeline-item">
      <div class="timeline-dot ${m.status === "Completed" ? "act-invoice" : "act-followup"}">${icon(MEETING_TYPE_ICON[m.type] || "meeting")}</div>
      <div class="timeline-content">
        <div class="timeline-title">${m.type} ${statusBadge(m.status)}</div>
        <div class="timeline-desc">${m.notes || "No notes added."} ${m.attachment ? `<span class="text-faint fs-xs">${icon("paperclip")} ${escapeHtml(m.attachment)}</span>` : ""}</div>
        <div class="timeline-time">${formatDate(m.date)} · ${m.time}</div>
        <div class="flex gap-2 mt-2">
          ${m.status !== "Completed" ? `<button class="btn btn-secondary btn-sm" data-mt-act="complete" data-mt-id="${m.id}">${icon("check")} Complete</button>` : ""}
          <button class="btn btn-ghost btn-sm" data-mt-act="edit" data-mt-id="${m.id}">${icon("edit")} Edit</button>
        </div>
      </div>
    </div>`).join("")}</div>` : emptyPanel("No meetings yet", "Schedule the first meeting for this lead.", null, null);

  document.querySelectorAll("[data-mt-act]").forEach(btn => {
    btn.addEventListener("click", () => {
      if (btn.dataset.mtAct === "complete") completeMeeting(btn.dataset.mtId);
      if (btn.dataset.mtAct === "edit") openMeetingModal(lead.id, btn.dataset.mtId);
    });
  });
}
...
function wireActions(lead) {
  ...
  document.getElementById("meetingLeadBtn").onclick = () => openMeetingModal(lead.id);
  ...
  if (!window.__leadDetailsEventsWired) {
    ...
    document.addEventListener("meeting:saved", () => renderMeetingTimeline(getLeadById(CURRENT_LEAD_ID)));
  }
}
